...
test('return wallet by walletId', function () {
    var phantomWallet = { walletId: types_1.WalletId.Phantom };
    var wallets = [[{ walletId: types_1.WalletId.Metamask }], [phantomWallet]];
    expect((0, useWeb3_1.byWalletId)(wallets, types_1.WalletId.Phantom)).toStrictEqual([phantomWallet]);
});
test('returns authenticated wallets', function () {
    var phantomWallet = { walletId: types_1.WalletId.Phantom, account: null };
    var metamaskWallet = { walletId: types_1.WalletId.Metamask, account: 1 };
    var walletConnectWallet = { walletId: types_1.WalletId.WalletConnect, account: 2 };
    var wallets = [
        [metamaskWallet],
        [phantomWallet],
        [walletConnectWallet],
    ];
    expect((0, useWeb3_1.getAuthenticated)(wallets)).toStrictEqual([
        [metamaskWallet],
        [walletConnectWallet],
    ]);
});
